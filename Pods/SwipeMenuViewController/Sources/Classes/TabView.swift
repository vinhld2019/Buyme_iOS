    ...
    fileprivate func setupAdditionView() {
        ...
        case .circle:
            DispatchQueue.main.async {
                ...
                let height = itemView.bounds.height - self.options.additionView.padding.vertical
                let width = itemView.bounds.width - self.options.additionView.padding.horizontal
                self.additionView = UIView(frame: CGRect(x: itemView.frame.origin.x + self.options.additionView.padding.left, y: 0, width: width, height: height))
                self.additionView.layer.position.y = itemView.layer.position.y
                self.additionView.layer.cornerRadius = self.options.additionView.circle.cornerRadius ?? self.additionView.frame.height / 2
                self.additionView.backgroundColor = self.options.additionView.backgroundColor
                self.containerView.addSubview(self.additionView)
                self.containerView.sendSubviewToBack(self.additionView)
            }
        case .none:
